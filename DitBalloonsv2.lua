...
if getgenv().cailonme.UltraBoost then
    ...
    game:GetService("Workspace").__THINGS.Ultimates:Destroy()
...
local function mail(item) -- parse item savefile eg.: save.magic_shards
    ...
    if item.name then
        if item.name == "Diamonds" then
            if arg5 > cailonme.Gems then
                args[5] = cailonme.Gems
...
if updateSave().diamonds.amount >= cailonme.Gems then
...
for i = 1, cailonme.HopSec do
    print("Times no.", i)
    Doing2.Text = "Find ur mom fat"
    baloonLoop()
    task.wait(0.5)
end
